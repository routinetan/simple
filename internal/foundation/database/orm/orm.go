...
import (
	...
	"simple/internal/foundation/unique"
	"sync/atomic"
	"time"
)
...
type (
	...
	connInfo struct {
		...
		MaxLifeTime int    `toml:"max_left_time"`
	}
...
func createConnectionURL(username, password, addr, dbName string) string {
	app2.Logger().Debug(fmt.Sprintf("%s:%s@tcp(%s)/%s?charset=utf8&parseTime=True&loc=Local", username, password, addr, dbName))
	return fmt.Sprintf("%s:%s@tcp(%s)/%s?charset=utf8&parseTime=True&loc=Local", username, password, addr, dbName)
}

// Start 启动数据库
func Start() {
	err = app2.Config().Bind("application", "database", &conf)
	if err == app2.ErrNodeNotExists {
		return
	}
	...
	if err != nil {
		app2.Logger().WithField("log_type", "foundation.orm.orm").Warn("database connect error, you can't use orm support: ")
		app2.Logger().WithField("log_type", "foundation.orm.orm").Warn(err)
	}
	...
	orm.Master.DB().SetConnMaxLifetime(time.Duration(conf.Master.MaxLifeTime))
