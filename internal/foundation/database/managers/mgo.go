...
import (
	...
	"simple/internal/foundation/database"
	"simple/internal/foundation/database/mgo"
...
func (manager *MgoManager) List(ctx *gin.Context) {
	result := pager2.New(ctx, pager2.NewMgoDriver()).SetIndex(manager.Table.TableName()).
		SetNextStartField("ID").SetPrevStartField("ID").Find(manager.Table).Result()
	app2.NewResponse(app2.Success, result, "SUCCESS").End(ctx)
}
...
func (manager *MgoManager) Post(ctx *gin.Context) {
	...
	if !validate.IsValid() {
		app2.NewResponse(app2.Fail, nil, "FAIL").End(ctx)
		return
	}
	...
	app2.NewResponse(app2.Success, insert, "SUCCESS").End(ctx)
	return
}
...
func (manager *MgoManager) Put(ctx *gin.Context) {
	id := ctx.PostForm("_id")
	if id == "" {
		app2.NewResponse(app2.Fail, nil, "OperateIdCanNotBeNull").End(ctx)
		return
	}
	...
	app2.NewResponse(app2.Success, newInstance.Interface(), "SUCCESS").End(ctx)
	return
...
func (manager *MgoManager) Delete(ctx *gin.Context) {
	...
	app2.NewResponse(app2.Success, nil, "SUCCESS").End(ctx)
}
