...
func Start() {
	var err error
	err = app2.Config().Bind("application", "mongo", &conf)
	...
	if err != nil {
		app2.Logger().WithField("log_type", "foundation.mongo.mongo").Error(err)
	}
...
func (collection *CollectionInfo) FindMany(documents interface{}) {
	...
	if val.Kind() != reflect.Ptr || val.Elem().Kind() != reflect.Slice {
		app2.Logger().WithField("log_type", "foundation.mongo.mongo").Error("result argument must be a slice address")
	}
...
func (collection *CollectionInfo) Delete() int64 {
	if collection.filter == nil || len(collection.filter) == 0 {
		app2.Logger().WithField("log_type", "foundation.mongo.mongo").Error("you can't delete all documents, it's very dangerous")
		return 0
	}
	ctx, _ := context.WithTimeout(context.Background(), 5*time.Second)
