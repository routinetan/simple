...
func (config config) ElasticSearchConfig() *esConfig.Config {
	err = app2.Config().Bind("application", "elasticsearch", &conf)
...
func Start() {
	...
	if err != nil {
		app2.Logger().WithField("log_type", "foundation.elastic.es").Error(err)
	}
}
...
func Insert(index string, body interface{}) *elastic.IndexResponse {
	...
	if err != nil {
		app2.Logger().WithField("log_type", "foundation.elastic.es").Error("es write error: ", err)
	}
